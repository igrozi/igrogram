...
const createPopParticles = (message, bubble, color) => {
  ...
  const speedProfiles = [
    {
      ...
      riseBoost: 0,
      easeSet: [
        "cubic-bezier(0.28, 0.72, 0.3, 1)",
        "cubic-bezier(0.3, 0.7, 0.32, 1)",
      ],
    },
    {
      durationMin: 1760,
      ...
      riseBoost: 10,
      easeSet: [
        "cubic-bezier(0.24, 0.7, 0.28, 1)",
        "cubic-bezier(0.22, 0.68, 0.3, 1)",
      ],
    },
    {
      durationMin: 2280,
      ...
      riseBoost: 18,
      easeSet: [
        "cubic-bezier(0.2, 0.62, 0.28, 1)",
        "cubic-bezier(0.18, 0.6, 0.26, 1)",
      ],
    },
  ];
  ...
  for (let index = 0; index < particleConfigs.length; index += 1) {
    ...
    const yRatio =
      config.kind === "large"
        ? 0.56 + (Math.random() - 0.5) * 0.16
        : 0.58 + (Math.random() - 0.5) * 0.24;
    const originX =
      bubbleRect.left - messageRect.left + bubbleRect.width * xRatio;
    const originY =
      bubbleRect.top - messageRect.top + bubbleRect.height * yRatio;
    ...
    const particleEase =
      speedProfile.easeSet[
        Math.floor(Math.random() * speedProfile.easeSet.length)
      ];
...
const createSplashDroplets = (message, bubble, color) => {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const angleOffset =
      (Math.floor(index / 2) / Math.max(1, Math.ceil(count / 2) - 1) - 0.5) *
      1.4;
  ...
  useEffect(() => {
    ...
    newSocket.on("messages_read", (data) => {
      if (data.receiver_id === user.user_id) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.sender_id === data.receiver_id
              ? { ...msg, is_read: true }
              : msg,
          ),
        );
  ...
  const sidebarTopItems = [
    {
      ...
      icon: User,
      label: "ПРОФИЛЬ",
      action: () => navigate(`/profile/${userProfile?.username}`),
    },
            ...
            <div className="flex-1 flex flex-col bg-white/90 dark:bg-slate-900/90 backdrop-blur-md overflow-hidden">
              <div className="p-4 border-b border-gray-200 dark:border-slate-800 shrink-0">
                <div className="relative">
                  {isSearching ? (
                    <Loader2
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-600 animate-spin"
                      size={18}
                    />
                  ) : (
                    <Search
                  ...
                  <>
                    <div className="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 border-b border-indigo-100 dark:border-indigo-800/30">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
                        ГЛОБАЛЬНЫЙ ПОИСК • {searchResults.length} НАЙДЕНО
                      </span>
                    </div>
                      ...
                      >
                        <div className="relative shrink-0">
                          ...
                          {checkIsOnline(contact) && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 bg-green-500"></div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <span className="font-black text-base text-gray-900 dark:text-white truncate block">
                            {contact.name}
                          </span>
                          <span className="text-xs font-black uppercase tracking-wider text-indigo-500 mt-0.5 block">
                            @{contact.username}
                          </span>
                        </div>
                      </div>
                  ...
                  <>
                    ...
                    {recentChats.map((contact) => {
                      ...
                      const lastMsgText = lastMsg?.body
                        ? lastMsg.body.length > 35
                          ? lastMsg.body.substring(0, 35) + "..."
                          : lastMsg.body
                        : lastMsg?.image_url
                          ? "📷 Изображение"
                          : "Нет сообщений";
          ...
          {activeMobileTab === "contacts" && (
            <div className="flex-1 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md overflow-y-auto overflow-x-hidden">
              <div className="px-4 py-3 bg-gray-100 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700">
                ...
                <span className="text-[10px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  ВСЕ КОНТАКТЫ •{" "}
                  {allUsers.filter((u) => u.user_id !== user.user_id).length}
                </span>
              </div>
              {allUsers
                ...
                .map((contact) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <span className="font-black text-base text-gray-900 dark:text-white block">
                        {contact.name}
                      </span>
                      <span className="text-xs font-black uppercase tracking-wider text-indigo-500">
                        @{contact.username}
                  ...
                  >
                    <img
                      src={
                        ...
                        `https://ui-avatars.com/api/?name=${selectedContact.name}&background=4f46e5&color=fff&size=128`
                      }
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${checkIsOnline(selectedContact) ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
                        />
                        <span className="text-[9px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">
                          {checkIsOnline(selectedContact)
                            ? "В СЕТИ"
                          ...
                          >
                            <div className="message-content relative z-0">
                              {msg.reply_to && (
                                ...
                                >
                                  <div className="flex items-center gap-1 mb-0.5">
                                    ...
                                    />
                                    <span
                                      className={`font-black text-[7px] uppercase tracking-wider ${isMe ? "text-indigo-200" : "text-gray-500"}`}
                                    >
                                      ОТВЕТ{" "}
                                      {msg.reply_to_sender === user?.name
                                        ? "СЕБЕ"
                                        : msg.reply_to_sender}
                                    </span>
                                  </div>
                                  <p
                                    className={`italic truncate text-[8px] ${isMe ? "text-indigo-100" : "text-gray-400"}`}
                                  >
                              ...
                              <div className="px-3 py-2 pb-5 relative min-w-[70px]">
                                {msg.body && (
                                  <p className="text-sm font-extrabold leading-relaxed whitespace-pre-wrap break-words">
                                    {msg.body}
                                  </p>
                                )}
                    ...
                    <div className="mb-2 relative inline-block">
                      <div className="bg-white dark:bg-slate-800 p-1 rounded-xl border-2 border-indigo-600 shadow-lg">
                        ...
                        <button
                          ...
                          className="absolute -top-1 -right-1 bg-red-500 text-white p-0.5 rounded-full border-2 border-white shadow-md hover:bg-red-600 transition-colors"
                        >
                          <X size={9} />
                        </button>
                      </div>
                  ...
                  >
                    <div className="flex-1 bg-gray-100 dark:bg-slate-800 rounded-2xl flex items-center gap-2 px-3 py-2">
                      ...
                      <input
                        type="text"
                        placeholder={
                          replyingTo ? "НАПИШИТЕ ОТВЕТ..." : "СООБЩЕНИЕ..."
                        }
                    ...
                    <button
                      type="submit"
                      disabled={
                        isSending || (!messageBody.trim() && !imageFile)
                      }
          ...
          {!selectedContact &&
            ...
            !showMobileChatList && (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h2 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-2">
                  ВЫБЕРИТЕ ЧАТ
                </h2>
                <p className="text-gray-500 dark:text-gray-400 text-[10px] font-black">
                ...
                {searchResults.map((contact) => (
                  ...
                  >
                    <div className="relative shrink-0">
                      ...
                      />
                      {checkIsOnline(contact) && (
                        <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-slate-900 bg-green-500"></div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className="font-black text-lg text-gray-900 dark:text-white truncate block">
                        {contact.name}
                      </span>
                      <span className="text-sm font-black uppercase tracking-wider text-indigo-500 mt-0.5 block">
                        @{contact.username}
                      </span>
                    </div>
                  </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            <span className="text-sm font-black uppercase tracking-wider text-indigo-500">
                              @{contact.username}
                            </span>
                          </div>
        ...
        <div className="flex-1 flex flex-col bg-white/50 dark:bg-slate-900/50 h-full overflow-hidden">
          {selectedContact ? (
                        ...
                        >
                          <div className="message-content relative z-0">
                            {msg.reply_to && (
                              ...
                              >
                                <div className="flex items-center gap-1 mb-0.5">
                                  ...
                                  />
                                  <span
                                    className={`font-black text-[8px] uppercase tracking-wider ${isMe ? "text-indigo-200" : "text-gray-500"}`}
                                  >
                                    ОТВЕТ{" "}
                                    {msg.reply_to_sender === user?.name
                                      ? "СЕБЕ"
                                ...
                                <p
                                  className={`italic truncate text-[9px] ${isMe ? "text-indigo-100" : "text-gray-400"}`}
                                >
                                  {msg.reply_to_body || "СООБЩЕНИЕ УДАЛЕНО"}
                                </p>
                              </div>
                            ...
                            <div className="px-4 py-2 pb-5 relative min-w-[80px]">
                              {msg.body && (
                                <p className="text-base font-extrabold leading-relaxed whitespace-pre-wrap break-words">
                                  {msg.body}
                                </p>
                              )}
                  ...
                  <div className="mb-2 relative inline-block">
                    <div className="bg-white dark:bg-slate-800 p-1 rounded-xl border-2 border-indigo-600 shadow-lg">
                      ...
                      <button
                        ...
                        className="absolute -top-1.5 -right-1.5 bg-red-500 text-white p-0.5 rounded-full border-2 border-white shadow-md hover:bg-red-600 transition-colors cursor-pointer"
                      >
                        <X size={10} />
                      </button>
                    </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              ...
              <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-2">
                ВЫБЕРИТЕ ЧАТ
              </h2>
              <p className="text-gray-500 dark:text-gray-400 text-xs font-black">
                ИСПОЛЬЗУЙТЕ ПОИСК, ЧТОБЫ НАЙТИ ДРУЗЕЙ
              </p>
            </div>
          )}
