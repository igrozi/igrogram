    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-md bg-white dark:bg-slate-900 border-4 border-red-500/30 rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center gap-4 mb-6 relative z-10">
            <motion.div 
              ...
              className="p-3 bg-red-500/10 rounded-2xl border-2 border-red-500/30"
            >
              <Trash2 size={24} className="text-red-500" />
            </motion.div>
            <div>
              <h3 className="text-lg md:text-xl font-black uppercase tracking-wider dark:text-white">Удаление аккаунта</h3>
              <p className="text-[9px] md:text-[10px] font-bold text-red-500 uppercase tracking-wider">Необратимое действие</p>
            </div>
          </div>
          
          <div className="p-3 md:p-4 bg-red-500/5 rounded-2xl border-2 border-red-500/20 mb-6 relative z-10">
            <p className="text-[10px] md:text-xs font-bold text-gray-600 dark:text-gray-300 uppercase leading-relaxed">
              <span className="text-red-500">⚠ ВНИМАНИЕ:</span> Все ваши данные, посты, сообщения и файлы будут безвозвратно удалены.
            </p>
          </div>
          
          <div className="relative z-10 mb-4">
            <label className="text-[9px] md:text-[10px] font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest ml-4 mb-2 block">
            ...
            <div className="relative">
              ...
              <input 
                ...
                placeholder="••••••••"
                className={`w-full bg-gray-100 dark:bg-slate-800 border-2 p-4 pl-12 pr-12 rounded-xl font-bold text-base dark:text-white outline-none transition-all ${
                  localError ? 'border-red-500' : 'border-gray-200 dark:border-slate-700 focus:border-red-500'
                }`}
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    handleConfirm();
            ...
            {localError && (
              <motion.p 
                ...
                className="text-red-500 text-[10px] font-bold uppercase mt-2 ml-4"
              >
                {localError}
              </motion.p>
            )}
          ...
          <div className="flex gap-3 relative z-10">
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              onClick={onClose} 
              className="flex-1 py-3 md:py-4 bg-gray-200 dark:bg-slate-800 text-gray-700 dark:text-gray-300 rounded-xl font-black uppercase tracking-widest text-xs md:text-sm hover:bg-gray-300 dark:hover:bg-slate-700 transition-colors"
            >
              ОТМЕНА
            </motion.button>
            <motion.button 
              ...
              disabled={loading} 
              className="flex-1 py-3 md:py-4 bg-gradient-to-r from-red-600 to-rose-600 text-white rounded-xl font-black uppercase tracking-widest text-xs md:text-sm hover:from-red-500 hover:to-rose-500 shadow-lg shadow-red-500/20 flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
    ...
    return (
      <div className={`min-h-screen ${darkMode ? 'dark bg-[#020617]' : 'bg-gray-100'} pb-20`}>
        ...
        <div className="relative z-10 px-4 py-4 space-y-4">
          ...
          {/* Кнопка удаления аккаунта для мобильной версии */}
          <motion.button 
            whileTap={{ scale: 0.98 }}
            type="button"
            onClick={() => setShowDeleteModal(true)}
            className="w-full p-4 rounded-2xl border-2 border-red-500/30 bg-red-500/5 text-red-500 font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2"
          >
            <Trash2 size={16} />
          ...
          <motion.button 
            ...
            className="w-full p-4 rounded-2xl border-2 border-red-500/20 text-red-500 font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2"
          >
...
// Desktop version - с блоком удаления в сайдбаре
return (
    ...
    >
      <div className="p-8 border-b-4 border-gray-300 dark:border-slate-800 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          <button 
            onClick={() => navigate(-1)} 
            className="p-3 bg-white/80 dark:bg-slate-900/50 border-2 border-gray-300 dark:border-slate-800 rounded-2xl hover:border-indigo-500 transition-all shadow-lg hover:scale-105 cursor-pointer backdrop-blur-sm"
          >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 space-y-4 flex flex-col no-scrollbar relative">
        ...
        <div onClick={() => setProfile({...profile, notifications: !profile.notifications})} className={`cursor-pointer p-5 rounded-[2rem] border-2 flex items-center justify-between transition-all relative overflow-hidden ${profile.notifications ? 'border-indigo-600 bg-indigo-600/5' : 'border-gray-200 dark:border-slate-800'}`}>
          ...
          <div className="flex items-center gap-3 relative z-10">
            <div className={`p-2.5 rounded-xl ${profile.notifications ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-200 dark:bg-slate-700 text-gray-500'}`}>
              <Bell size={18} />
            </div>
        ...
        {/* БЛОК УДАЛЕНИЯ АККАУНТА - В САЙДБАРЕ */}
        <motion.div 
          whileHover={{ scale: 1.01 }}
          className="p-5 bg-red-500/5 rounded-[2rem] border-2 border-red-500/20 relative overflow-hidden group hover:border-red-500/40 transition-all cursor-pointer"
          onClick={() => setShowDeleteModal(true)}
        >
          <div className="absolute -right-4 -bottom-4 opacity-[0.05] pointer-events-none">
            <Trash2 size={60} className="text-red-500" />
          </div>
          <div className="flex items-center gap-3 relative z-10">
        ...
        <motion.button 
          ...
          whileTap={{ scale: 0.99 }}
          onClick={handleLogOut} 
          className="w-full p-4 text-red-500 bg-transparent border-2 border-red-500/20 rounded-[2rem] font-black uppercase tracking-widest shadow-lg hover:bg-red-500/10 hover:border-red-500/40 transition-all flex items-center justify-center gap-3 cursor-pointer text-sm"
        >
        ...
        <div className="mt-auto p-5 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-[2rem] bg-white/50 dark:bg-slate-900/50 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-2 text-indigo-600 dark:text-indigo-400 relative z-10">
            <LockKeyhole size={16} />
              ...
              <div className="flex flex-col relative z-10">
                ...
                <div className="relative mb-4">
                  ...
                  <input 
                    ...
                    onChange={e => setSecureData({...secureData, newPassword: e.target.value})} 
                    placeholder="НОВЫЙ ПАРОЛЬ" 
                    className="w-full bg-gray-100 dark:bg-slate-800 border-4 border-gray-200 dark:border-slate-700 p-6 pl-16 rounded-[2rem] font-bold text-lg dark:text-white focus:border-indigo-600 outline-none transition-all placeholder:text-gray-400 placeholder:text-sm" 
                  />
                  <button 
                    type="button" 
                    onClick={() => setShowPass(!showPass)} 
                ...
                {secureData.newPassword && (
                  <div className="relative">
                    ...
                    <input 
                      ...
                      onChange={e => setSecureData({...secureData, confirmPassword: e.target.value})} 
                      placeholder="ПОДТВЕРДИТЕ ПАРОЛЬ" 
                      className={`w-full bg-gray-100 dark:bg-slate-800 border-4 p-6 pl-16 rounded-[2rem] font-bold text-lg dark:text-white focus:border-indigo-600 outline-none transition-all placeholder:text-gray-400 placeholder:text-sm ${
